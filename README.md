...
- **无线调试**：通过 ADB 协议命令一键开启/关闭 5555 端口无线调试（非 root 可用）。
- **设备信息**：连接后自动读取机型、Android 版本、当前电量与 SELinux 状态。
- **自定义命令**：支持输入任意 shell 命令（自动过滤粘贴内容里多余的 `adb shell` 前缀，按回车直接执行）。
- **纯前端运行**：没有后端，不收集数据，通过浏览器原生 WebUSB 接口直连，依赖已本地打包，无外部请求。
- **多语言**：根据浏览器语言自动进入中文或英文页面。

## 部署

项目没有构建步骤，不需要安装 Node.js。将 `index.html` 与 `vendor/` 目录一起放到任意静态服务器（如 GitHub Pages、Cloudflare Pages 或 Nginx）即可（页面已内置中英文切换，无需额外文件）。
